...
public class MedicamentoController {
	...
	// 
	public String detailsMedicamentoPost(@PathVariable("id") long id, @Valid Medicamento medicamento, BindingResult result,RedirectAttributes attributes) {
		if(result.hasErrors()) {
			attributes.addFlashAttribute("mensagem", "Verifique os campos");
			return "redirect:/{id}";
		}
		
		Optional<Medicamento> existingMedicamento = mr.findById(id);
		if (existingMedicamento == null) {
		    attributes.addFlashAttribute("mensagem erro", "Medicamento não encontrado para atualização");
		    return "redirect:/{id}";
		}
		
		mr.save(medicamento);
		attributes.addFlashAttribute("mensagem sucesso", "Medicamento atualizado!");
	...
	public String updateMedicamento(@Valid Medicamento medicamento, BindingResult result, RedirectAttributes attributes) {
	    if (result.hasErrors()) {
	        attributes.addFlashAttribute("mensagem", "Verifique os campos...");
	        return "redirect:/editMedicamento";
	    }
